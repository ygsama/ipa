...
public class BookController {
    ...
    public ResponseEntity<Book> updateBook(@PathVariable("id") String id, @RequestBody Book updateBook) {
        Book book = bookRepository.findBookById(id);
        if ("".equals(updateBook.getId())) {
            book.setId(updateBook.getId());
        }
        if ("".equals(updateBook.getName())) {
            book.setName(updateBook.getName());
        }
        if ("".equals(updateBook.getAuthor())) {
